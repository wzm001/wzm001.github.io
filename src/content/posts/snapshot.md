...
publishedAt: "2023-03-06 10:45:16 +8:00"
category: "Databases"
...
快照隔离需要使用长写锁来防止脏写。（参阅[[database-transactions]]）。因此意味着进行写入的事务会阻止其他事务修改相同的对象。但读取则不需要加锁，从性能的角度来看，快照隔离的关键原则是：**读不阻塞写，写不阻塞读**。
...
快照隔离不能防止 **丢失更新** 。[[database-transactions]]
