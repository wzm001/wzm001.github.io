...
publishedAt: "2023-05-05 21:50:16 +8:00"
...
2. 如果是业务层面合法的请求，但后台 _暂时_ 没有数据，可以考虑将 `null` 缓存下来。但这时候需要注意，这个缓存是临时性的，因为后续可能该数据就不为空了。所以缓存的时候可以给个稍微短一些的过期时间，比如 5 分钟。并且当数据库写入或更新数据时，需要同时刷新缓存以避免数据不一致。
